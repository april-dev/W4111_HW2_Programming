...
class CSVTable:
    ...
    def __get_access_path__(self, fields):
        ...
        count = None
        result = None
        for index in self.__description__.indexes:
            if set(index.column_names).issubset(set(fields)):
                if count is None or len(self.__indexes__[index.index_name]) > count :
                    count = len(self.__indexes__[index.index_name])
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        result = []
        for index in self.__description__.indexes:
            if index.index_name == idx_name:
                key = self.__get_key__(index, t)
                result = self.__indexes__[idx_name][key]
                return result
        return result
    ...
    def __find_by_template__(self, template, fields=None, limit=None, offset=None):
        ...
        if indexes is None or indexes == {}:  # also should mean a derived table or some other issue
            ...
            result_index, count = self.__get_access_path__(list(template.keys()))
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        # ************************ TO DO ***************************
        left_r = self
        left_where_template = left_r.__get_sub_where_template__(where_template)
        right_where_template = right_r.__get_sub_where_template__(where_template)
        left_rows = []
        right_rows = []
        for r in left_r.__get_row_list__():
            if left_r.matches_template(r, left_where_template):
                left_rows.append(r)
        for r in right_r.__get_row_list__():
            if right_r.matches_template(r, right_where_template):
                right_rows.append(r)
        result_rows = []

        left_r.__reload__(left_rows)
        right_r.__reload__(right_rows)
        l_result_index, l_count = left_r.__get_access_path__(on_fields)
        r_result_index, r_count = right_r.__get_access_path__(on_fields)

        if r_result_index is not None and l_result_index is not None:
            left_selectivity = left_rows/l_count
            right_selectivity = right_rows / r_count
            if (left_selectivity<right_selectivity):
                temp = left_rows
                left_rows = right_rows
                right_rows = temp
        elif r_result_index is None and l_result_index is not None:
            temp = left_rows
            left_rows = right_rows
            right_rows = temp
        ...
        left_rows_processed = 0
        for lr in left_rows:
            on_template = left_r.__get_on_template__(lr, on_fields)
            right_filtered = right_r.__find_by_template__(on_template)
            for rr in right_filtered:
                new_r = {**lr, **rr}  # appends two dictionaries together
                result_rows.append(new_r)
            left_rows_processed += 1
            if left_rows_processed % 10 == 0:
        ...
        join_result = self.__table_from_rows__("JOIN:" + left_r.__table_name__ + ":" + right_r.__table_name__, result_rows)
        result = join_result.__find_by_template__(template=None, fields=project_fields)  # join table won't have indexes so it will use template_scan
