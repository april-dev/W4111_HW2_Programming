...
class TableDefinition:
        ...
        if not load:
            ...
            if index_definitions is not None:
                for idx in index_definitions:
                    self.define_index(idx.index_name, idx.index_type, idx.column_names)
    ...
    def load_columns(self):
        ...
        self.columns = []
        for row in result:
            n_null = False if row['not_null'] == 0 else True
            newRow = ColumnDefinition(column_name=row['column_name'], column_type=row['type'], not_null=n_null)
            self.columns.append(newRow)
    ...
    def load_indexes(self):
        ...
        self.indexes = []
        for row in result:
            index = self.get_index(row['index_name'])
            if index:
                self.get_index(row['index_name']).add_column(row['column_name'])
            else:
                self.indexes.append(IndexDefinition(row['index_name'], row['type'], [row['column_name']]))
    ...
    def add_column_definition(self, c):
        ...
        print("Column '" + c.column_name + "' is added")
    ...
    def define_index(self, index_name, columns, type="index"):
        ...
        if self.indexes is None:
            self.indexes = []
        self.indexes.append(IndexDefinition(index_name, type, columns))
        print("Index '" + index_name + "' has been added")
    ...
    def drop_index(self, index_name):
        ...
        if index_to_drop is not None:
            ...
            self.drop_index_in_sql(index_name)  # You must implement this function
            print("Index '" + index_name + "' has been dropped!")

        return

    def drop_index_in_sql(self, index_name):
        """
...
class CSVCatalog:
    ...
    def __init__(self, dbhost="127.0.0.1", dbport=3306,
                 dbuser="root", dbpw="april661123", db="CSVCatalog", debug_mode=None):
